...
<div id="wrapper">
	<div id="content">
		<div class="s_wrap">
			...
			<form method="post" action="/admin/modifyFaqs">
						...
						<tbody>
							...
							<tr>
								...
								<td colspan="3"><select name="category" id="category"
									required>
										<option value="채용일반"
											<c:if test="${faqs.category == '채용일반'}">selected</c:if>>채용
											일반</option>
										<option value="채용과정"
											<c:if test="${faqs.category == '채용과정'}">selected</c:if>>채용
											과정</option>
										<option value="지원서작성"
											<c:if test="${faqs.category == '지원서작성'}">selected</c:if>>지원서
											작성</option>
										<option value="시스템오류"
											<c:if test="${faqs.category == '시스템오류'}">selected</c:if>>시스템
											오류</option>
								</select></td>
							</tr>
							...
							<tr>
								...
								<td colspan="3"><input type="text" name="title" required
									value="${faqs.title}" class="frm_input" size="100"></td>
							</tr>
